...
public class DecodeWays {
    public int numDecodings(String s) {
        ...
        //ways[s.length()] will be answer
        // ways[0] = 1; //empty string
        ways[0] = chars[0] != '0' ? 1 : 0; //if it is non-zero, it gets decoded
        for (int i = 1; i < s.length(); ++i) {
            //single digit
            if (chars[i] != '0') //chars i one behind ways i
                ways[i] = ways[i - 1]; //if non-zero, then at least nmber of ways till i-1 valid as chars[i] also valid
            //double digit
            ...
            if (doubleDigit >= 10 && doubleDigit <= 26)
                ways[i] += ways[i - 1]; //all ways till i-2 also valid as chars[i-1]chars[i] is also an integer
        }
