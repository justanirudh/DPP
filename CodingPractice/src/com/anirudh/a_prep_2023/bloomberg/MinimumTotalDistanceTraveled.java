...
public class MinimumTotalDistanceTraveled {
    //TODO
    //Maybe can be done by parallel BFS
    Long[][] dp;
    ...
    public long solve(List<Integer> list, int[][] arr, int i, int j) {
        ...
        if (i >= list.size())
            return 0L;  // if all robot are repaired then there is no robot left to repair so return 0.
        if (j >= arr.length)
            return Long.MAX_VALUE; // here we check if there is no factory left for rapair robot but we have some robot to repair because we didnt pass on first condition.
