...
Tx (??) : mn * (2^mn); for each cell (m*n) we are traversing through all possible combinations to get Neighbours (2^mn)
...
public class MinimumNumberofFlipstoConvertBinaryMatrixtoZeroMatrix {
    ...
    static class State {
            ...
            for (int i = 0; i < mat.length; ++i) {
                for (int j = 0; j < mat[0].length; ++j) {
                    if ((i == x && j == y) ||
                            ...
                            (i == x && j == y - 1) ||
                            (i == x + 1 && j == y) ||
                            (i == x && j == y + 1)) { //same cell or neighbour
        ...
        List<State> getNeighbours() { //O(mn * mn)
